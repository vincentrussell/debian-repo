...
openssl pkcs12 -in server.p12 -nocerts -nodes -passin pass:password | openssl rsa > server.id_rsa
openssl pkcs12 -in server.p12 -clcerts -nokeys -passin pass:password  | openssl x509 -pubkey -noout | ssh-keygen -f /dev/stdin -i -m PKCS8 > server.id_rsa.pub
chmod 600 server.id_rsa
chmod 600 server.id_rsa.pub
